...
public class ContestControllerTest {
  ...
  void getAllContests_ShouldReturnList() throws Exception {
    ...
    mockMvc
        .perform(get("/api/contests/all").header(HttpHeaders.AUTHORIZATION, jwtToken))
        .andExpect(status().isOk())
  ...
  void getAllContests_ShouldFail_WhenNoToken() throws Exception {
    mockMvc.perform(get("/api/contests/all")).andExpect(status().isForbidden());
  }
  ...
  void getContest_ShouldReturn404_WhenParamIsMissing() throws Exception {
    mockMvc
        .perform(get("/api/contests/").header(HttpHeaders.AUTHORIZATION, jwtToken))
        .andExpect(status().isNotFound());
  }
